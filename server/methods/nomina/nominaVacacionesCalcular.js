...
import { NominaHeaders_sql } from '/server/imports/sqlModels/nomina/nomina'; 
...
Meteor.methods({
    ...
    nominaVacacionesCalcular: function (vacacion0) {
        ...
        let query = '';
        query = `Select Top(1) h.GrupoNomina as grupoNomina From tNominaHeaders h Inner Join tNomina n On h.ID = n.HeaderID
                        Where n.Empleado = ? And h.FechaNomina <= ? Order By h.FechaNomina Desc
                `;

        response = null;
        response = Async.runSync(function(done) {
            sequelize.query(query, { replacements: [ vacacion.empleado, moment(vacacion.regreso).format('YYYY-MM-DD'), ],
        ...
        for (const g of response.result) { 
            grupoNomina = g.grupoNomina ? g.grupoNomina : null;
            break; 
        }
