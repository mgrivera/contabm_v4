...
Meteor.methods(
{
    'nomina.catalogos.parentescos.save': function (items) {
        ...
        inserts.forEach(function (item) {
            ...
            response = Async.runSync(function(done) {
                Parentescos_sql.create(item_sql)
            ...
            item.parentesco = savedItem.parentesco     // recuperamos el id (pk) de la cuenta; pues se le asignó un valor en sql ...
            Parentescos.insert(item, function (error, result) {
                if (error) {
        ...
        updates.forEach(function (item) {
            ...
            response = Async.runSync(function(done) {
                Parentescos_sql.update(item_sql, { where: { parentesco: item_sql.parentesco }})
                    .then(function(result) { done(null, result); })
                    .catch(function (err) { done(err, null); })
                    .done();
            })
            ...
            Parentescos.update({ _id: item._id }, { $set: item.object }, {}, function (error, result) {
        ...
        removes.forEach(function (item) {
            ...
            response = Async.runSync(function(done) {
                Parentescos_sql.destroy({ where: { parentesco: item.parentesco }})
                    .then(function(result) { done(null, result); })
                    .catch(function (err) { done(err, null); })
                    .done();
            })
            ...
            Parentescos.remove({ _id: item._id });
        })
