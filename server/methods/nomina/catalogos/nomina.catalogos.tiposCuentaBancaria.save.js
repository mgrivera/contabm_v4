...
Meteor.methods(
{
    'nomina.catalogos.tiposCuentaBancaria.save': function (items) {
        ...
        inserts.forEach(function (item) {
            ...
            response = Async.runSync(function(done) {
                TiposDeCuentaBancaria_sql.create(item_sql)
            ...
            item.tipoCuenta = savedItem.tipoCuenta     // recuperamos el id (pk) de la cuenta; pues se le asignó un valor en sql ...
            TiposDeCuentaBancaria.insert(item, function (error, result) {
                if (error) {
        ...
        updates.forEach(function (item) {
            ...
            response = Async.runSync(function(done) {
                TiposDeCuentaBancaria_sql.update(item_sql, { where: { tipoCuenta: item_sql.tipoCuenta }})
                    .then(function(result) { done(null, result); })
                    .catch(function (err) { done(err, null); })
                    .done();
            })
            ...
            TiposDeCuentaBancaria.update({ _id: item._id }, { $set: item.object }, {}, function (error, result) {
        ...
        removes.forEach(function (item) {
            ...
            response = Async.runSync(function(done) {
                TiposDeCuentaBancaria_sql.destroy({ where: { tipoCuenta: item.tipoCuenta }})
                    .then(function(result) { done(null, result); })
                    .catch(function (err) { done(err, null); })
                    .done();
            })
            ...
            TiposDeCuentaBancaria.remove({ _id: item._id });
        })
