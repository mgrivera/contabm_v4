...
Meteor.methods(
{
    'nomina.catalogos.cargos.save': function (items) {
        ...
        inserts.forEach(function (item) {
            ...
            item.cargo = savedItem.cargo     // recuperamos el id (pk) de la cuenta; pues se le asignó un valor en sql ...
            Cargos.insert(item, function (error, result) {
                if (error) {
        ...
        updates.forEach(function (item) {
            ...
            response = Async.runSync(function(done) {
                Cargos_sql.update(item_sql, { where: { cargo: item_sql.cargo }})
                    .then(function(result) { done(null, result); })
                    .catch(function (err) { done(err, null); })
                    .done();
            })
            ...
            Cargos.update({ _id: item._id }, { $set: item.object }, {}, function (error, result) {
        ...
        removes.forEach(function (item) {
            ...
            response = Async.runSync(function(done) {
                Cargos_sql.destroy({ where: { cargo: item.cargo }})
                    .then(function(result) { done(null, result); })
                    .catch(function (err) { done(err, null); })
                    .done();
            })
