...
Meteor.methods(
{
    'bancos.cuentasContablesDefinicionSave': function (items) {
        ...
        inserts.forEach(function (item) {
            ...
            delete item_sql.nombreCompania; 
            delete item_sql.descripcionCuentaContable; 
            delete item_sql._id;
            delete item_sql.user;

            response = Async.runSync(function(done) {
                DefinicionCuentasContables_sql.create(item_sql)
            ...
            Temp_Consulta_Bancos_CuentasContables_Definicion.insert(item, function (error, result) {
                if (error)
        ...
        updates.forEach(function (item) {
            ...
            response = Async.runSync(function(done) {
                DefinicionCuentasContables_sql.update(item_sql, { where: { claveUnica: item_sql.claveUnica }})
                    .then(function(result) { done(null, result); })
                    .catch(function (err) { done(err, null); })
                    .done();
            })
            ...
            Temp_Consulta_Bancos_CuentasContables_Definicion.update({ _id: item._id }, { $set: item.object }, {}, function (error, result) {
        ...
        removes.forEach(function (item) {
            ...
            response = Async.runSync(function(done) {
                DefinicionCuentasContables_sql.destroy({ where: { claveUnica: item.claveUnica }})
                    .then(function(result) { done(null, result); })
                    .catch(function (err) { done(err, null); })
                    .done();
            })
            ...
            Temp_Consulta_Bancos_CuentasContables_Definicion.remove({ _id: item._id });
        })
