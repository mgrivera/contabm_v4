...
Meteor.methods(
{
    bancosDeterminarAnosCerrados: function (ciaContab) {
        // debugger;

        Match.test(ciaContab, Match.Object);
        Match.test(ciaContab.numero, Match.Integer);

        query = `Select Distinct Ano as ano
                 From Saldos s Inner Join CuentasBancarias c On s.CuentaBancaria = c.CuentaInterna
                 Where c.Cia = ?`;
