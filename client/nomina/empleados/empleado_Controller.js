...
import { Empleados_SimpleSchema } from '/imports/models/nomina/empleados'; 
...
function ($scope, $stateParams, $state, $meteor, $modal, uiGridConstants) {
    ...
    const companiaContabSeleccionada = $scope.$parent.companiaSeleccionada; 
    ...
    $scope.refresh = () => {
        ...
        // desde sql server ...
        empleados_leerByID_desdeSql($scope.empleado.empleado);
    }
    ...
    $scope.grabar = function () {
        ...
        let editedItem = lodash.cloneDeep($scope.empleado);
        ...
        if (editedItem.docState != 3) {
            isValid = Empleados_SimpleSchema.namedContext().validate(editedItem);

            if (!isValid) {
                Empleados_SimpleSchema.namedContext().validationErrors().forEach(function (error) {
                    errores.push("El valor '" + error.value + "' no es adecuado para el campo '" + Empleados_SimpleSchema.label(error.name) + "'; error de tipo '" + error.type + "'.");
                });
            }
        }
        ...
        Meteor.call('empleadosSave', editedItem, (err, result) => {
            ...
            empleados_leerByID_desdeSql(parseInt(result.id));
        })
    }
    ...
    function empleados_leerByID_desdeSql(pk) {
        ...
        Meteor.call('empleados_leerByID_desdeSql', pk, (err, result) => {
            ...
            if (!$scope.empleado || lodash.isEmpty($scope.empleado)) {
